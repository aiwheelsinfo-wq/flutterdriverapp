...
class _CarFormPageState extends State<CarFormPage> {
  ...
  void _populateRcDetails(Map<String, dynamic> details, String rcNo) {
  ...
  Future<void> _showRcOtpDialog(String clientId, String mobileNumber, String rcNo) async {
        ...
        return StatefulBuilder(
          builder: (context, setDialogState) {
            return AlertDialog(
              ...
              title: Row(
                children: const [
                  Icon(Icons.shield_outlined, color: primaryAmber),
                  SizedBox(width: 8),
                  Expanded(
                    child: Text(
                      "Vehicle Owner OTP Verification",
              ...
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    Text(
                      "OTP sent to RTO Registered Mobile: $mobileNumber",
                ...
                TextButton(
                  onPressed: isSubmittingOtp
                      ...
                      : () {
                          ...
                          setState(() {
                            isVerifyingRc = false;
                            rcVerificationStatusMessage = "⚠️ OTP Verification Cancelled by User";
                            rcVerificationStatusColor = Colors.orange;
                          });
                        },
                      ...
                      : () async {
                          ...
                          try {
                            ...
                            if (response.statusCode == 200 && resData['success'] == true) {
                              Navigator.pop(dialogContext);
                              final details = resData['data'] ?? {};
                              _populateRcDetails(details, rcNo);
                            } else {
                              setDialogState(() {
                                isSubmittingOtp = false;
                                otpError = resData['message'] ?? "Invalid OTP. Please try again.";
                              });
                            }
                          } catch (e) {
                            setDialogState(() {
                              isSubmittingOtp = false;
                              otpError = "Error: ${e.toString().replaceAll('Exception:', '').trim()}";
                            });
                          }
                        },
  ...
  Future<void> _onVerifyRc() async {
    ...
    try {
      ...
      if (response.statusCode == 200 && data['success'] == true) {
        if (data['already_verified'] == true && data['data'] != null) {
          _populateRcDetails(data['data'], rcNo);
        } else if (data['otp_sent'] == true) {
          final clientId = (data['client_id'] ?? '').toString();
          final mobileNum = (data['mobile_number'] ?? 'RTO Mobile').toString();

          setState(() {
            isVerifyingRc = false;
            rcVerificationStatusMessage = "📩 OTP sent to $mobileNum. Please enter OTP.";
            rcVerificationStatusColor = primaryAmber;
          });

          await _showRcOtpDialog(clientId, mobileNum, rcNo);
        } else if (data['data'] != null) {
          _populateRcDetails(data['data'], rcNo);
        } else {
          setState(() {
            isVerifyingRc = false;
            rcVerificationStatusMessage = "⚠️ ${data['message'] ?? 'RC details ready'}";
          });
        }
      } else {
